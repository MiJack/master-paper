...
同时也在调用图中添加方法参数、方法实例对应的对象节点$o_p$、$o_i$以及方法对象和方法的关系$ (  o_p \stackrel{parameter}{\longrightarrow}   m )  $、$ (  o_i \stackrel{instance}{\longrightarrow}   m ) $（第9$\sim$10行）。
如果此时当前线程栈$stack$的栈顶元方法素$top$存在，系统会创建从方法$top$到当前方法$m$的调用关系，$\left(top \to m \right) $，并将当前方法$m$压入栈$stack$中（第11$\sim$ 14行）。
	...
	\FOR {thread $\in$ $logs.threads$}
		...
		\FOR  {$log$ $\in$ $logs.get(thread)$}
				...
				\IF {isMethodStartLog($log$)} 
					...
					\STATE 	$cg$.addMethodObjectRels($ (  o_p \stackrel{parameter}{\longrightarrow}   m )  $,$ (  o_i \stackrel{instance}{\longrightarrow}   m ) $) 
