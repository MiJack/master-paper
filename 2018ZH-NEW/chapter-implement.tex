	...
	\SetKwProg{Fn}{Function}{:}{}
			...
			\For{thread $\in$ $logs.threads$}{
				...
				\eIf{isMethodStartLog($log$)} {
				   ...
				   %\Comment 在调用图中提交方法节点 
				  	...
				  	$cg$.addMethodObjectRels($ \left\langle  o_p \stackrel{parameter}{\longrightarrow}   m \right\rangle   $,$ \left\langle  o_i \stackrel{instance}{\longrightarrow}   m \right\rangle  $) ;
			...
			%	\Comment 在调用图中提交方法对象节点 （此处只涉及参数关系和实例关系）
				...
				$stack$.push($m$);
					 ...
					 $cg$.addMethodObjectRel($ \left\langle   o_r \stackrel{return}{\longrightarrow}   m \right\rangle  $) ;
...
% $o_p \stackrel{parameter}{\longrightarrow} m$
	\Fn{addThreadTrigger($ecg$)} {
		\For{\emph{$o_r \in \{ o \mid o$  is the $Runnable$ object node $\in$ $ecg \}$} }{
			\If{ $ \left\langle m_{start}\stackrel{instance}{\longrightarrow} o_r \right\rangle \in ecg$  \\\qquad 
					and   $\left\langle m_{run} \stackrel{instance}{\longrightarrow}   o_r\right\rangle  \in ecg$ }{
				$ecg$.addThreadTriggerRel($m_{start} \hookrightarrow m_{run} $)	
			}
			\If{ $ \left\langle m_{runOnUiThread} \stackrel{parameter}{\longrightarrow}   o_r \right\rangle \in ecg$  \\\qquad   
				and   $\left\langle m_{run} \stackrel{instance}{\longrightarrow}   o_r\right\rangle  \in ecg$  }{
	...
	\Fn{addHandlerTrigger($ecg$)} {
		\For{\emph{ $o_m \in  \{ o\mid$ o is the $Message$ object node $\in$ $ecg \}$} }{
				\If{ $ \left\langle m_{enqueue}\stackrel{parameter}{\longrightarrow} o_m\right\rangle \in ecg$  \\\qquad   
					and   $\left\langle m_{dispatch}\stackrel{parameter}{\longrightarrow} o_m\right\rangle  \in ecg$ }{
