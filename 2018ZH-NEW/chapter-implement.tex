...
%由于预处理器可以处理的方法需要
\todo {由于无法对系统方法的源代码进行直接修改，因此预处理器无法实现对系统方法的日志代码编织。}
...
关系生成器是第\ref{chp:design}章中\autoref{alg:buildCG}、\autoref{alg:buildActivityLifecycle}、\autoref{alg:buildTrigger}的具体实现，\todo{基于Cypher脚本和Soot完成}。
例如，\autoref{alg:buildTrigger}第16$\sim$17行中，我们需要找到使用$o_{m}$作为参数的方法$m_{enqueue}$、$m_{dispatch}$，并建立两者之间的触发关系。
传统的实现方式需要对图进行遍历，效率较低。\todo{而RunDroid采用了Cypher脚本，实现简洁，开发效率高，如\autoref{fig:cypher_code}所示。}
Soot的工作主要是提供应用程序相关的\todo{类继承检索}查询服务。
