...
%并在此基础上，我们介绍了RunDroid中各组成模块的实现细节。
...
%在遍历框架中，我们介绍了我们实现的类图，并详述了所有模块的实现方法和技术手段。
%其中，我们在使用原型工具中，发现了很多技术问题，如被测App跳出自己的package和Android5.0的权限管理系统。
%我们通过建立守护进程的方式对这问题加以解决。
%我们在开发过程中，注意模块化设计，方便进一步研究后的迭代开发。
%我们开发过程中，将App状态抽象、概率生成、和执行器三个模块做了逻辑分离。
%将模块独立化可以使得当未来有更好解决方案时，我们可以做到快速迭代更新。
%其次，我们介绍了监控性能分析的技术实现。
%我们依赖Xposed对App做了无感的入侵，劫持了Activity和Fragment类中的方法，通过计算时间戳的方式获取执行时长。
%同时，我们也依赖Xposed技术，通过修改阈值来使用Android原生的方式输出掉帧信息，最后我们介绍了较为成熟的基础性能指标的监控。
