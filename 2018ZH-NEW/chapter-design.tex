...
\begin{algorithm}[!ht]
	...
	\Fn{patchUiEvent($cg$)} {
		\For{\emph{$o_{listener} \in \{ o \mid o $  is the $View.OnClickListener$ object node ,$o \in$ $cg \}$} }{		
			\For{\emph{$o_{view} \in \{ o \mid o $  is the $View$ object node ,$o \in$ $cg \}$} }{
				\If{ {$ \left\langle m_{register}\joinrel\xrightarrow{instance} o_{view}\right\rangle \in cg$ } \\\qquad 
					and   {$\left\langle m_{register} \joinrel\xrightarrow{parameter}   o_{listener}\right\rangle  \in cg$ }			 \\\qquad 
							and   {$\left\langle m_{click} \joinrel\xrightarrow{parameter}   o_{view}\right\rangle  \in cg$ }	 \\\qquad 					
