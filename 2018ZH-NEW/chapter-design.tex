...
\begin{algorithm}[!ht]
	...
	\Fn{patchActivityLifecycle($ecg$,$manifestFile$)}{
		...
		\For{ \emph{ $o_{activity} \in \{ activity \mid activity $ defined in  $manifestFile \} $ }}{
			...
			\For{ \emph{ $m \in \{ m \mid  m$ is method node in $ ecg $    \\\qquad
...
\begin{algorithm}[!ht]
	...
	\Fn{addTriggerRels($ecg$)}{
		...
		\tcp{addThreadTrigger into $ecg$}
		
		
		\For{\emph{$o_t \in \{ o \mid o $  is the $Thread$ object node ,$o \in$ $ecg \}$} }{
			\If{ \emph{$o_t$,$m_{start}$ and $m_{run}$  satisfy formula \ref{equ:rule_thread}}}{
