...
\begin{algorithm}[!hb]
	...
	\Fn{patchActivityLifecycle($ecg$,$manifestFile$)}{
		...
		\For{ \emph{ $o_{activity} \in \{ activity \mid activity $ defined in  $manifestFile \} $ }}{
			...
			\For{ \emph{ $m \in \{ m \mid  m$ is method node in $ ecg $, $o_{activity}$ is instance of $m\}$}}{
	...
	\Fn{patchUiEvent($ecg$)} {
		\For{\emph{$o_{view} \in \{ o \mid o $  is the $View$ object node ,$o \in ecg \}$} }{
			\For{\emph{$o_{listener} \in \{ o \mid o $  is the $View.OnClickListener$ object node,  $ o \in ecg \}$} }{		
				\If {\emph{$o_{view}$,$o_{listener}$,$m_{register}$ and $m_{click}$  satisfy formula \ref{equ:rule_ui}}
					}{
...
\begin{algorithm}[!ht]
	...
	\Fn{addTriggerRels($ecg$)}{
		...
		{// add the Thread Trigger into $ecg$}
		
		
		\For{\emph{$o_{thread} \in \{ o \mid o $  is the $Thread$ object node ,$o \in$ $ecg \}$} }{
			\If{ \emph{$o_{thread}$,$m_{start}$ and $m_{run}$  satisfy formula \ref{equ:rule_thread}}}{
		...
		\For{\emph{$o_r \in \{ o \mid o $  is the $Runnable$ object node ,$o \in$ $ecg \}$} }{
			\If{ \emph{$o_r$,$m_{runOnUiThread}$ and $m_{run}$  satisfy formula \ref{equ:rule_runOnUiThread}}}{
				$ecg$.addTriggerRel($m_{runOnUiThread}  \lhook\joinrel\xrightarrow{runOnUiThread}  m_{run} $)	
			}	
		}
		{// add the Handler Trigger into $ecg$}
		
		
			\For{\emph{ $o_m \in  \{ o\mid$ o is the $Message$ object node ,$o \in$ $ecg \}$} }{
			\If{ \emph{$o_m$,$m_{handle}$ and $m_{dispatch}$  satisfy formula \ref{equ:rule_handler}}}{
