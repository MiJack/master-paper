...
另外，在函数调用图的基础上，RunDroid利用日志中包括的方法对象信息，挖掘和方法对象相关联的方法，结合触发规则，进而建立方法触发关系。
...
但是两者最终的效果是一致的：当应用程序运行时，相关方法的执行信息均会以日志的形式记录下来，用于后续调用图的构建。
...
列表中的方法通常和Android特性相关，涵盖Activity的生命周期方法、Java Thread API以及Handler等多个方面。
...
最后，将列表$lifecycleList$按照时间顺序进行排序，并依次连接起来，即可得到\code{Activity}的生命周期（第12$\sim$13行）。
...
例如，在重写方法\code{onCreate()}时，我们会调用\code{super.onCreate()}以完成Activity窗体的初始化。
