...
\section{用户方法执行信息的获取——基于源代码的日志代码插桩过程}
...
预处理器会遍历项目源代码中的每一个Java文件，将对应利用srcML转化成对应的抽象语法树。
	...
	%	$cg'  \gets  cg$;
		...
		\For{ \emph{ $m \in \{ m \mid  m$ is the method node in $ cg \}$}}{
			\If{ \emph{ $m$ is the method of Activity's Lifecycle   \\\qquad  
						and   \todo{$  \left\langle  m' \to m \right\rangle  \notin cg$} }}{
...
\begin{algorithm}[!ht]
	...
	\Fn{addThreadTrigger($ecg$)} {
		\For{\emph{$o_r \in \{ o \mid o$  is the $Runnable$ object node $\in$ $ecg \}$} }{
			\If{ \todo{$ \left\langle m_{start}\joinrel\xrightarrow{instance} o_r \right\rangle \in ecg$ } \\\qquad 
				and   \todo{$\left\langle m_{run} \joinrel\xrightarrow{instance}   o_r\right\rangle  \in ecg$ }}{
				$ecg$.addThreadTriggerRel($m_{start} \hookrightarrow m_{run} $)	
			}
			\If{ \todo{$ \left\langle m_{runOnUiThread} \joinrel\xrightarrow{parameter}   o_r \right\rangle \in ecg$}  \\\qquad   
				and  \todo{ $\left\langle m_{run} \joinrel\xrightarrow{instance}   o_r\right\rangle  \in ecg$  }}{
	...
	\Fn{addHandlerTrigger($ecg$)} {
		...
		\For{\emph{ $o_m \in  \{ o\mid$ o is the $Message$ object node $\in$ $ecg \}$} }{
			\If{\todo{ $ \left\langle m_{enqueue}\joinrel\xrightarrow{parameter} o_m\right\rangle \in ecg$}  \\\qquad   
				and  \todo{ $\left\langle m_{dispatch}\joinrel\xrightarrow{parameter} o_m\right\rangle  \in ecg$ }}{
