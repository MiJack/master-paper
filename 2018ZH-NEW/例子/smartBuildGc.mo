package smartBuildGc
  ...
  model smartBuildingIni
    ...
    Real humans[n](start = {1e-30, 0, 1e-30, 1e-30, 1e-30});
    ...
    Integer roomNeeds;
    //需要加热的房间数目
    Real allUn(start = 0.0), energy(start = 0.0);
    Real derAllUn(start = 0.0);
    Real derEnergy(start = 0);
  ...
  algorithm
    ...
    /*for ii in 1:3 loop
                              if heater[ii] < 1 then
                                ...
                                h[i] := 1;
                                rh[i] := ii;
                                roomNeeds:=if roomNeeds > 0 then roomNeeds-1 else roomNeeds;
                                break;
                              end if;
                            end for;*/
    //roomNeeds:=if roomNeeds > 0 then roomNeeds-1 else roomNeeds;
    if roomNeeds > 0 then
      ...
      sortUn := sort(5, unCom);
      for i in 1:n loop
        if need[sortUn[i]] == 1 and h[sortUn[i]] == 0 then
          for j in 1:m loop
            if heater[j] == 0 then
              heater[j] := sortUn[i];
              h[sortUn[i]] := 1;
    ...
    for i in 1:m loop
      if heater[i] > 0 then
        derEnergy := derEnergy + power[heater[i]];
      end if;
    end for;
