...
通过方法$m_{start}$触发方法$m_{run}$的执行时，方法$m_{start}$和$m_{run}$的实例对象均为同一个\code{Thread}对象$o_{thread}$。
...
因此，当两个方法$m_{start}$ 、$m_{run}$及\code{Thread}对象$o_{thread}$满足\autoref{equ:rule_thread}时，$m_{start} \lhook\joinrel\xrightarrow{Thread}  m_{run}  $ 成立。
...
根据第\ref{chp:background}章中关于\code{Handler}的介绍，我们知道\code{Handler}通过\code{Message}进行多线程消息调度：
