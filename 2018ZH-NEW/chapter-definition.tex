...
从\autoref{fig:handler-apis}、\autoref{fig:handler-framework}中可知，所有的Handler API最后就会调用到\code{Handler.enqueueMessage( MessageQueue, Message, long)}方法，即$m_{send} \stackrel{\ast	}{\Rightarrow} m_{enqueue} $。

以及$m_{dispatch} \stackrel{\ast}{\Rightarrow}  m_{handle}$，
