...
针对Android 组件\code{Activity}的生命周期特性，FlowDroid 进行了针对性的建模，将Activity的生命周期和UI事件回调串联起来，构造应用层面的方法\code{dummyMainMethod()}表示应用的执行过程，相应的结果如\autoref{fig:flowdroid-result-lifecycle}所示。
