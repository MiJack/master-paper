...
实验相关配置的环境如下：
PC端为ThinkPad E430，CPU为Intel 酷睿i5 3210M，内存为12GB。
手机型号为小米 MI 5，处理器型号为骁龙820，RAM为3GB，系统为Android 6.0，内置Xposed运行环境。




\section{函数触发关系的普遍性验证}
...
\begin{figure*}[!hb]
	...
	\caption{RunDroid产生的方法doFibbonaci()有关的调用图}
...
\begin{figure*}[!hb]
	...
	\caption{FlowDroid产生的函数doFibbonaci()有关的调用图}
...
\subsection{Activity 生命周期和事件回调的效果对比}
...
\begin{figure*}[!ht]
	...
	\caption{表示Activity生命周期和事件回调的状态变迁图（RunDroid）}
...
\begin{figure*}[ht]
	...
	\caption{函数dummyMainMethod()的控制流图}
...
\subsection{多线程触发关系的效果对比}

%多线程开发是Android开发中过程的典型开发要求。
本文针对Android中的多线程开发场景，我们进行了相关的测试。
在本节，我们将点击按钮button3，获取\code{doHandleButton3()}相关的调用图情况。
RunDroid和FlowDroid的运行结果分别如\autoref{fig:rundroid-result-handler}、\autoref{fig:flowdroid-result-handler}所示，
两者在以下几个方面存在不同。
...
\begin{figure*}[ht]
	...
	\caption{RunDroid产生的多线程触发关系有关的调用图}
...
\begin{figure*}[ht]
	...
	\caption{FlowDroid产生的多线程触发关系有关的调用图}
...
在WorkerThread的方法run()中，我们通过Handler进行了异步UI操作，触发了方法\code{MainActivity\$1.handleMessage(Message)}的执行\footnote{由于我们定义的Handler是MainActivity的匿名内部类，因此编译后的类名为\code{MainActivity\$1}。}。
...
\subsection{错误定位技术原理简介}



基于因果关联模型的错误定位技术，以程序本身$P$和一组测试用例的覆盖率信息作为输入，
通过期望模型（\autoref{equ:expectEquation}）和线性回归模型（\autoref{equ:linearEquation}）计算程序$P$中的语句$s$的错误估计值$\tau$。
...
\begin{table}[!ht]
	...
	\begin{center}
		...
		\begin{tabular}{|rl*{5} {|c}|c|c|}
			...
			&                                                  & \textbf{0} & \textbf{1} & \textbf{0} & \textbf{0} &\textbf{ 1} &   &               \\
									\hline
		\end{tabular}
	\begin{tablenotes}[flushleft]
	\item	注： 第2列到第6列分别代表测试用例$t_1\sim t_5$；
		每个测试用例列的标题分别显示在第1行和第2行使用的v和num的值；
		测试用例列的值表示相应的程序语句是否在该测试用例执行，1表示覆盖（$T=1$），0表示未覆盖（$T=0$）；   
		最后一行表示各个测试用例的执行结果，“\textbf{1}”表示测试未通过（$Y=1$），“\textbf{0}”表示测试通过（$Y=0$）。 % \par
		\end{tablenotes}
			\end{threeparttable}
...
\Line{12}的前驱节点也发生了变化，但是由于实验组和对照组匹配结果保持不变，因此结果保持不变。
