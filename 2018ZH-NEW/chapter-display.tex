...
编写的5个测试用例$t_1\sim t_5$中，$t_2$、$ t_5$未通过测试。
...
\Line{13} 分数提高的原因如下：
在接入RunDroid前，方法\code{onClick(View)}和\code{TaskThread.run()}没有任何关系（此处指的调用关系）。
...
\Line{13}的前驱节点增加了\Line{2}，\Line{5}和\Line{9}，实验组和对照组的\code{Mdata(\Line{13)}数据如\autoref{fig:motivationResultTreatment}-(b)所示。
...
\end{center}
	\caption{语句Line.8 Units in treatment group and control group with their covariate values}
