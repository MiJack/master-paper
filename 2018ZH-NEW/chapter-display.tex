...
\begin{algorithm}[!ht]
	...
	\Fn{FaultLocazilation($P$,$CoverageInfo$)}{
		...
		\For{$s \in P$ }{
			...
			Model($s$) = Compute causal model of $s$;
			
			 Pred($s$) = Compute predecessors of s from Model($s$);
			
			  Mdata($s$) = Match on Pred($s$) in $CoverageInfo$;
			  
			\eIf{\emph{Mdata($s$) = $\emptyset$	}}{
				  Compute $\tau(s)$ from Mdata($s$) using Equation $\tau(s) = E[Y^1] - E [Y^0]$;}
			  {
%				  Y = α + τT + βX + ε
%				$\tau(s) = NA$;
				
			  Compute $\tau(s)$ using Equation $\tau(s) =\alpha +\tau T+\beta X+\varepsilon $;
			 }
