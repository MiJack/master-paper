...
在Android应用程序中，逻辑代码分散在不同方法中，通过系统调度在时间维度上串联起来，\todo{响应时间的}。


\todo{
由于我们对\autoref{table:app_results}的应用进行了简单的交互，
因此，期间触发的函数触发关系有限，RunDroid捕获到的函数触发关系大都集中在10$\sim$39之间。
}
\todo{在这个方面，}应用osmtracker-android和	archwiki-viewer属于两个不同的极端。
...
因此，\todo{静态分析技术得到的函数调用图往往以方法体本身作为研究的基本单元}，而动态分析技术的函数调用图可以细化方法执行之间的关系，反映了程序执行的具体过程。
...
出现这个现象的原因如下：该方法属于系统定义的方法，而运行时拦截器待拦截的方法列表中并未包含该方法，因此运行时未收集到相关的方法执行信息，导致RunDroid无法在调用图中还原相应的\todo{函数}。
这也反映了RunDroid的不足：关于系统函数的运行时信息的捕获需要提前将相应的方法告知运行时拦截器，生成的调用图才会包括相应的方法。
\todo{这使得调用图只包含研究人员关心的方法的执行信息，但也在一定程度上导致部分方法的缺失和调用图的局部不完整。}
...
通过查阅文献\cite{gosling2000java}，\todo{我们发现出于提高字符串串联性能的考虑，Java编译器可以使用类\code{StringBuilder}等技术通过源代码做适当等价的修改}，以避免表达式求值过程中产生过多的中间字符串对象。
...
最终，我们得到的RunDroid的运行结果如\autoref{fig:rundroid-result-lifecycle}所示\footnote{由于Neo4j表示的函数调用图无法完整地展示方法签名，\autoref{fig:rundroid-result-lifecycle}根据RunDroid产生的函数调用图重新绘制得到。}。
针对Android 组件\code{Activity}的生命周期特性，FlowDroid 进行了针对性的建模，通过构造方法\code{dummyMainMethod()}\todo{将Android Activity的生命周期和UI事件回调串联起来}，相应的结果如\autoref{fig:flowdroid-result-lifecycle}所示。
...
相比FlowDroid，RunDroid的运行结果是程序运行时的直接反映，\todo{更适合反映应用的状态变化}。
...
\footnote{由于Neo4j表示的函数调用图无法完整地展示方法签名，\autoref{fig:rundroid-result-handler}根据RunDroid产生的函数调用图重新绘制得到。}。
两幅图在以下节点上存在不同:
...
\todo{当方法\code{Thread.start()}和\code{WorkerThread.run()}同时在同一个函数中被调用，FlowDroid在调用图上无法将这两个方法间的调用关系和触发关系区分开。
...
\todo{进而形成数据依赖关系}。
\Line{13}的前驱节点增加了\Line{2}、\Line{5}和\Line{9}，\todo{变为}，实验组和对照组的\code{Mdata(\Line{13})}如\autoref{fig:motivationResultTreatment}-(b)所示。
实验组中的测试用例$t_3$在对照组中找不到对应的测试用例，因此不参与计算。
最后，根据测试用例$t_1$、$t_2$、$t_4$、$t_5$得到\Line{13}的错误估计值：$\tau =  \frac{1+1	}{2} - \frac{0+0}{2} = 1$。
\todo{介绍一下\Line{12}，为什么不变。}
