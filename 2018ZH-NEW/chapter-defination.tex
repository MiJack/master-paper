...
\end{myDef}
	...
	\begin{itemize}
		\item 若对象$o_p$是这个方法$m$的参数，记为$o_p \stackrel{parameter}{\longrightarrow} m$，或者 $ rel(m,o_p) = parameter$，或者三元组$\left\langle m, o_p, parameter\right\rangle $；
		\item 若对象$o_r$是这个方法$m$的返回值，记为$o_r \stackrel{return}{\longrightarrow} m$，或者 $ rel(m,o_r) = return$，或者三元组$\left\langle m, o_p, return\right\rangle $；
		\item 若方法$m$是非静态方法，则方法执行时我们可以获取到关联到的this指针对象$o_i$，记为$o_i \stackrel{instance}{\longrightarrow} m$，或者 $ rel(m,o_i) = instance$，或者三元组$\left\langle m, o_p, instance\right\rangle $；
	\end{itemize}
