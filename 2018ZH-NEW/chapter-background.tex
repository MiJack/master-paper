...
除了示例中的调用方式，Android SDK提供的API\cite{HandlerA26:online}还提供多种API，同时支持基于\code{Runnable}的消息调度和基于逻辑代码的消息调度：
开发人员可以通过\code{post(Runnable)}，\code{postAtTime(Runnable，long)}，\code{sendMessage(Message)}，
