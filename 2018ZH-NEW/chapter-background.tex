...
具体的，在工作线程中，用户生成的字符串和对应的逻辑代码\code{MSG\_CHANGE\_TEXT}封装到{Message}对象中，通过Handler发送出去；
Handler在主线程中收到了该{Message}对象时，会根据逻辑代码（即\code{Message.what}的值）决定进行对应的逻辑处理（本例中，对应逻辑为更新界面信息）。
...
除了示例中的调用方式，Android 系统提供的API\cite{HandlerA26:online}还提供多种API（如\autoref{fig:handler-apis}所示），同时支持基于{Runnable}的消息调度和基于逻辑代码的消息调度：
