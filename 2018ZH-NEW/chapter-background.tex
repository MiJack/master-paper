...
Android系统为\code{Activity}提供了\code{onCreate()}, \code{onStart()},\code{ onResume()}, \code{onPaused()},\code{ onRestart()}, \code{ onStoped()}和\code{onDestroy()}等方法，方便开发人员在\code{Activity}的状态发生变化时对程序的运行时数据和应用状态做适当的处理操作。
...
当开发人员需要当前业务逻辑转移到其他线程时，通过方法\code{Message.obtain()}获取一个Message，将对应的业务逻辑封装成\code{Runnable}对象传递给Message中对应的字段，
...
从Android SDK提供的API来看，开发人员可以通过\code{post(Runnable)}，\code{postAtTime(Runnable，long)}，\code{sendMessage(Message)}，\code{postDelayed(Runnable, Object, long)},
