...
\code{Activity}提供的方法包括\code{onCreate()}、 \code{onStart()}、\code{ onResume()}、 \code{onPaused()}、\code{ onRestart()}、 \code{ onStoped()}和\code{onDestroy()}等，方便开发人员在\code{Activity}的状态发生变化时对程序的运行时数据和应用状态做适当的处理操作。
...
\code{onRestart()}、\code{onStart()}、\code{onResume()}等方法被回调；
