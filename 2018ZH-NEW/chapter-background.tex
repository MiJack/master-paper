...
为此，Android系统在Java API的基础上还提供了\code{void runOnUiThread (Runnable action) }API。
\code{runOnUiThread} API可以帮助开发人员将业务逻辑的执行从工作线程转移到主线程上，该API也符合Android只允许在主线程上更新界面这一基本设计原则。
但是，该API也存在着一些弊端，例如runOnUiThread API的定义位于类\code{android.app.Activity}，这也就意味着在Android组件Service中进行耗时操作时，无法通过该API返回到主线程；同时基于接口的函数参数定义方式对于跨线程的参数传递也不是十分友好。
