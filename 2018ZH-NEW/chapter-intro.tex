...
\begin{figure*}[h]
	\centering
	\includegraphics[width=0.93\textwidth]{./Figures/app-numbers.png}
	\caption{Google Play Store上架应用数的变化趋势图}
...
静态分析技术通常以二进制程序文件或者程序源代码作为研究主体，结合控制流分析、数据流分析技术、指针分析以及程序依赖分析\cite{stafford2000formal}，得出应用程序相关的函数调用图、UML类图和序列图等，并在此基础上进行学术研究。
...
为此，德国达姆施塔特工业大学的Steven Arzt等人在Soot的基础上考虑Android程序中Activity的生命周期特性，推出了一个针对Android的静态分析工具FlowDroid\cite{arzt2014flowdroid}。%，可以做到上下文、路径、对象、字段等层面上的敏感分析。
...
在质量保障领域，分析技术可以帮助提升软件测试的代码覆盖率，辅助完成程序错误的定位、修复与分析。
%\cite{tan2018repairing,QingGaoASE15}以及变异测试\cite{MutationOperatorsAndroid,deng2015towards,linares2017enabling}等。
...
文献[\citenum{wong2009survey},\citenum{wong2016survey}]介绍上众多错误定位技术错误定位技术。
其中最有为经典的就是基于程序频谱的错误定位（SBFL），包括Tarantula\cite{jones2005empirical}、Jaccard\cite{jain1988algorithms}、Zoltar\cite{janssen2009zoltar}等。
%SBFL技术将程序语句的覆盖信息作为输入，利用故障模型公式计算语句的可疑性分数。
%语句的得分越高，这条语句越有可能是程序错误。
%研究人员还通过对比实验评估了不同故障模型公式的实验效果。
在SBFL的基础上，Baah等提出了基于程序依赖图的错误定位模型\cite{baah2010causal,baah2011mitigating}，通过引入因果关系模型，计算语句出错的可能性。
Mzoltar\cite{machado2013mzoltar}第一次使用SBFL技术Zoltar应用在Android上，实现了Android程序上的故障定位。
Mirzaei等人\cite{mirzaei2015exception} 将切片技术和Tarantula和Jaccard等技术结合用于错误定位，根据可疑性分数对语句进行排名。
文献[\citenum{fan2018efficiently}]提出的APEChecker通过静态分析技术和动态UI探索技术相结合，可以有效地检测出Android中的异步编程错误。 
文献[\citenum{tan2018repairing,QingGaoASE15}]等主要聚焦如何在Android应用程序出现问题时，根据异常的堆栈信息结合软件变异技术或者问答网站生成补丁代码，实现程序的自动修复。
范等人\cite{fan2018large}第一次对开源应用的issue进行了大规模研究，描述了Android应用程序中框架相关的异常，给出了11种开发人员可能遇到的常见异常。
