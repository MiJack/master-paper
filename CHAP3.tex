...
\define{
    调用关系（Invoke）：对于程序$P$的两个方法$m_1$和$m_2$，如果在方法$m_1$中调用了方法$m_2$，则记作$m_1 \to m_2$，称为方法$m_1$调用方法$m_2$。
}
\begin{equation}
m_0 \to m_1 \to \dots m_{n - 1} \to m_n  \label{equ:extend_invoke}
\end{equation}

    在此基础上，对于方法$m_0$和方法$m_n$，若存在方法$m_i$（$i=1,\dots,n-1，n>1$），使得~\autoref{equ:extend_invoke}成立，，则记作$m_0 \stackrel{\ast}{\to} m_n$，称为方法$m_0$扩展调用方法$m_n$。
